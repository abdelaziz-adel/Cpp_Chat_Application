...
public:
	...
	LambdaEventFilter(std::function<bool(QObject*, QEvent*)> cb, QObject* parent = nullptr)
...
void project_Gui::register_Screen(QWidget* window) {
	...
	QObject::connect(register_Button, &QPushButton::clicked, [=]() {
		...
		std::string firstName = fullName.toStdString().substr(0, firstSpace);
		size_t lastSpace = fullName.toStdString().rfind(' ');
		std::string lastName = fullName.toStdString().substr(lastSpace + 1);
...
void project_Gui::chat_Screen(QWidget* window) {
							...
							if (room.getChatRoomID() == sharedRoomID) {
								...
								QObject::connect(deleteBtn, &QToolButton::clicked, [=]() {
									...
									if (rightClickContainer && alignmentContainer && messageId != 0) {
										...
										cout << "error,invalid" << std::endl;
										if (!rightClickContainer)
											cout << "rightClickContainer is null" << std::endl;
										if (!alignmentContainer)
											cout << "alignmentContainer is null" << std::endl;
										if (messageId == 0)
											cout << "messageId is 0" << std::endl;
									}
									});
										...
										[=](QObject* watched, QEvent* event) -> bool {
											if (event->type() == QEvent::MouseButtonPress) {
												...
												if (mouseEvent->button() == Qt::RightButton && msg.getUserID() == currentUserID) {
													...
													////////////////////SAMA////SAMA////SAMA////SAMA////SAMA//////////////////////////
													cout << "Right-click on message ID: " << msg.getMessageID() << std::endl;
	...
	QObject::connect(searchField, &QLineEdit::textChanged, [=](const QString& text) {
		...
		std::sort(sortedInfos.begin(), sortedInfos.end(), [](const ContactInfo& a, const ContactInfo& b) {
...
void project_Gui::story_Screen(QWidget* window) {
	...
	std::set<int> currentUserContacts;
		...
		if (tmp_1 && tmp_2) {
			...
			if ((user->story.content != "" || user->story.photoPath != "") && user->profile.visibility && isUnopened) {
				...
				QObject::connect(contactBtn, &QPushButton::clicked, [=, &scrollContentConst]() {
					...
					if (isUnopened) {
						if (std::find(user->story.viewers.begin(), user->story.viewers.end(), currentUserID) == user->story.viewers.end()) {
	...
	for (auto& [id, user] : users) {
		...
		if (tmp_1 && tmp_2) {
			bool isOpened = std::find(user->story.viewers.begin(), user->story.viewers.end(), currentUserID) != user->story.viewers.end();
...
void project_Gui::setting_Screen(QWidget* window) {
	...
	QToolButton {
		...
		saveUsersToJson(); // Save the theme preference
		});
	...
	QToolButton {
		...
		saveUsersToJson(); // Save theme selection
		});
...
void project_Gui::group_Screen(QWidget* window) {
	...
	vector<std::string> predefinedColors = {
	"#FF0000", // Red
	"#008000", // Green
	"#0000FF", // Blue
	"#FFA500", // Orange
	...
	for (const auto& room : users[currentUserID]->chatRooms) {
		if (room.type() == "Group" && !processedChatRooms.contains(room.getChatRoomID())) {
			...
			int selectedID = -1;
			// Pick the first non-current user for avatar and number
			...
			if (selectedID == -1) continue; // Skip if no other users (shouldn't happen)
			...
			QObject::connect(contactBtn, &QPushButton::clicked, [=]() {
				...
				// Update message statuses and display messages
				for (ChatRoom& room : users[currentUserID]->chatRooms) {
					if (room.getChatRoomID() == selectedChatRoom) {
						vector<int> userIDs = room.getUserIDs();
						vector<Message> currentUserMessages = room.getMessages();
						...
						// Add current user to viewedBy for messages they didn't send
						for (Message& msg : updatedMessages) {
							if (msg.getUserID() != currentUserID) {
								msg.addViewer(currentUserID);
								// Check if all non-sender group members have viewed the message
								vector<int> viewedBy = msg.getViewedBy();
								bool allViewed = true;
								for (int userId : userIDs) {
									if (userId != msg.getUserID()) { // Exclude sender
	...
	QLabel* infoName = new QLabel(infoPopUp);
	//infoName->setGeometry(10, 120, 280, 150);
	...
	QLabel* infoNumber = new QLabel(infoPopUp);
	//infoNumber->setGeometry(10, 120, 280, 150);
...
void project_Gui::createGroupChat(const vector<int>& selectedUserIDs) {
	...
	allUserIDs.push_back(currentUserID); // Add current user
...
int project_Gui::customized_chat(const QString& message, QWidget* parent, int chatRoomID) {
	...
	vector<Message> updatedMessages; // To store the updated messages vector
...
QPushButton* project_Gui::customizedPop_Contact(int number, QString name, QWidget* parent, int y) {
	...
	QString Qnumber = QString::fromStdString(std::to_string(number));
...
void project_Gui::deleteMessage(int messageId, QWidget* alignmentContainer, int chatRoomID) {
	...
	for (auto& [userId, user] : users) {
		for (ChatRoom& room : user->chatRooms) {
			if (room.getChatRoomID() == chatRoomID) {
				room.removeMessage(messageId); // Use the new method
				messageDeleted = true; // Assume deletion succeeded (you can enhance this if needed)
			}
		}
	}
	...
	if (messageDeleted) {
		...
		cout << "Message with ID " << messageId << " deleted." << std::endl;
	}
	else {
		cout << "Message with ID " << messageId << " not found!" << std::endl;
	}
	///////////////////////SAMA////SAMA////SAMA////SAMA////SAMA////SAMA/////////////////////////
}
...
void project_Gui::removeExpiredStories() {
	...
	for (auto& pair : users) {
		...
		if (!user->story.publishedTime.empty()) {
			...
			if (publishedTime.isValid() && publishedTime.secsTo(currentTime) > 86400) { // 24 hours = 86400 seconds
				user->story.content.clear();
				user->story.photoPath.clear();
				user->story.publishedTime.clear();
				user->story.viewers.clear();
				std::cout << "Expired story deleted for user: " << pair.first << std::endl;
			}
		}
	}
